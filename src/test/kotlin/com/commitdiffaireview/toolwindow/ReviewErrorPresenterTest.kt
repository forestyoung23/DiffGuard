...
class ReviewErrorPresenterTest {
    ...
    fun `maps unauthorized http errors to api key guidance`() {
        val state = presenter.present(IllegalStateException("AI Review 请求失败：HTTP 401 unauthorized"))

        assertEquals("AI Review 请求失败", state.title)
        assertEquals("请检查 API Key 是否正确，或确认当前账号有权限访问该模型。", state.nextStep)
    }

    @Test
    fun `maps forbidden http errors to api key guidance`() {
        val state = presenter.present(IllegalStateException("AI Review 请求失败：HTTP 403 forbidden"))
    ...
    fun `maps not found http errors to base url or model guidance`() {
        val state = presenter.present(IllegalStateException("AI Review 请求失败：HTTP 404 model not found"))
    ...
    fun `maps rate limit http errors to retry guidance`() {
        val state = presenter.present(IllegalStateException("AI Review 请求失败：HTTP 429 too many requests"))
    ...
    fun `defaults missing error messages to generic failure detail`() {
        ...
        assertEquals("AI Review 失败。", state.detail)
    }
