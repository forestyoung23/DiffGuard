...
DiffGuard 是一个 IntelliJ IDEA 插件，用于在提交代码前对选中的 Git 变更执行 AI Code Review。插件调用用户配置的 OpenAI-compatible API，不运营托管审查服务。
...
- `API Key`：API Key，会保存到 IntelliJ PasswordSafe；设置页会以密码形式隐藏已保存的 Key。
...
- 插件不会主动发送数据到 DiffGuard 作者控制的服务器，除非用户显式把这类服务配置为 Provider。
- 用户应只配置自己信任的 OpenAI-compatible API Provider，并确认团队允许将相关代码片段发送给该 Provider。

## 使用方式

1. 在 Git 中选中或 stage 需要审查的文件。
2. 在 Commit/VCS 相关入口点击 `Review with DiffGuard`。
3. 插件会打开 `DiffGuard` ToolWindow，并显示审查进度。
...
DiffGuard 使用 [MIT License](LICENSE)。
